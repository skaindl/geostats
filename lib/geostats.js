...
var geostats = function(a) {
	...
	this.setClassManually = function(array) {
	    ...
	    if(array[0] !== this.min() || array[array.length-1] !== this.max()) {
	    	this.log(_t('Given bounds may not be correct! please check your input.\nMin value : ' + this.min() + ' / Max value : ' + this.max()));
	    	return; 
	    }
	...
	this.getClassGeometricProgression = function(nbClass) {
	    ...
	    if(this._hasNegativeValue() || this._hasZeroValue()) {
	    	this.log(_t('geometric progression can\'t be applied with a serie containing negative or zero values.'));
	    	return;
	    }
	...
	this.getClassJenks = function(nbClass) {
		...
		// to 0:
		if (kclass[0] == kclass[1] && 0 < kclass[1]) {
			kclass[0] = 0
		}
	...
	this.getHtmlLegend = function(colors, legend, counter, callback, mode) {
		...
		var cnt= '', i, ccolors, lg, getcounter, fn;
		...
		if(mode == 'discontinuous') {
			...
			if(this.counter.indexOf(0) !== -1) {
				this.log(_t("Geostats cannot apply 'discontinuous' mode to the getHtmlLegend() method because some classes are not populated.\nPlease switch to 'default' or 'distinct' modes. Exit!"));
				return;
			}
		...
		if(ccolors.length < this.ranges.length) {
			this.log(_t('The number of colors should fit the number of ranges. Exit!'));
			return;
		}
		...
		if(this.is_uniqueValues == false) {
			...
			for (i = this.bounds.length - 1; i >= 0; i--) {
				if(getcounter===true) {
					cnt = ' <span class="geostats-legend-counter">(' + this.counter[i] + ')</span>';
				}
